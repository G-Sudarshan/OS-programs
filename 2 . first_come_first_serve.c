// C program for implementation of First Come First Serve Scheduling Algorithm ( for Non pre-emptive processes )

//Github link : https://github.com/G-Sudarshan/

// 16 07 2019

// Second practical program [ for GP Nashik CM Department ]
...
int main()
{
    ...
    */
    ...
    for(i = 0 ; i < no ; i++)
    {
        ...
        printf("Enter order of process %d (Order in which process will come) : ",i+1);
        scanf("%d",&p[i].order);
    ...
    printf("values are : \n\n\n\n");
    printf("\n\nprocess name   order     burst time   waiting time   \n\n"); // you can also print arrival and completion time here

    for(i = 0 ; i < no ; i++ )
    {
    printf("   %s            %d           %d             %d       \n\n",p[i].p_name,p[i].order,p[i].burst_time,p[i].w_time);
    }
